...
const TutorialUI = () => {
  ...
  const handleHideUI = () => {
    game?.uiManager.displayUI(UIType.MAIN_MENU);
  };
      ...
      <div className="tutorial-container">
        ...
        <div className="tutorial-content">
          ...
          <div className="tutorial-section">
            ...
            <div className={`tutorial-section-content ${!currentSectionData.hasImages ? 'text-only' : ''}`}>
                ...
                <div className="tutorial-images-container">
                  {currentSectionData.images.map((image, index) => (
                    <div key={index} className="tutorial-image-wrapper">
                      <img 
                        src={image.path} 
                        alt={image.caption || `${currentSectionData.title} image ${index + 1}`}
                        className="tutorial-image"
                      />
                      {image.caption && <p className="tutorial-image-caption">{image.caption}</p>}
                    </div>
                  ))}
                </div>
              )}
              <div className={`tutorial-description ${!currentSectionData.hasImages ? 'full-width' : ''}`}>
